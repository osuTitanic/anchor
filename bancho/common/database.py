...
class Postgres:
    ...
    def user_by_name(self, name: str) -> Optional[DBUser]:
        return self.temp_session.query(DBUser) \
    ...
    def beatmap_by_id(self, id: int) -> Optional[DBBeatmap]:
        return self.temp_session.query(DBBeatmap) \
    ...
    def personal_best(self, beatmap_id: int, user_id: int, mode: int) -> Optional[DBScore]:
        return self.temp_session.query(DBScore) \
    ...
    def top_scores(self, user_id: int, mode: int, exclude_approved: bool = False) -> List[DBScore]:
        query = self.temp_session.query(DBScore) \
                        .filter(DBScore.user_id == user_id) \
                        .filter(DBScore.mode == mode) \
                        .filter(DBScore.status == 3)
